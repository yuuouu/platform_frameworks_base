...
public class Resources {
    ...
    /**
     * 通过该 XmlResourceParser 读取给定资源 ID 的视图布局描述。
     * 该解析器功能有限——特别是无法修改其输入，且仅提供高级事件。
     * 
     * <p>此函数实际上是调用 {@link #getXml} 并传入布局资源的简单封装。
     * 
     * @param id 所需的资源标识符，由 aapt 工具生成。
     *           此整数编码了包、类型和资源条目。值为 0 时为无效标识符
     *
     * @throws NotFoundException 如果给定的 ID 不存在，则抛出 NotFoundException
     * 
     * @return 一个新的解析器对象，通过它可以读XML 数据
     *         
     * @see #getXml
     */
    @NonNull
    ...
    /**
     * 通过该 XmlResourceParser 读取给定资源 ID 的动画描述。
     * 该解析器功能有限——特别是无法修改其输入，且仅提高级事件。
     * 
     * <p>此函数实际上是调用 {@link #getXml} 并传入动画资源的简单封装。
     * 
     * @param id 所需的资源标识符，由 aapt 工具生成。
     *           此整数编码了包、类型和资源条目。值为 0 时为无效标识符。
     *
     * @throws NotFoundException 如果给定的 ID 不存在，则抛出 NotFoundException
     * 
     * @return 一个新的解析器对象，通过它可以读取 XML 数据
     *         
     * @see #getXml
     */
    @NonNull
    ...
    XmlResourceParser loadXmlResourceParser(@AnyRes int id, @NonNull String type)
            throws NotFoundException {
         // 1. 获取一个临时的TypedValue对象。TypedValue是一个“值容器”，用于从底层接收资源的原始值信息（类型、数据、字符串等）。
        final TypedValue value = obtainTempTypedValue();
        try {
			// 2. 获取实际的资源实现对象（ResourcesImpl）
            final ResourcesImpl impl = mResourcesImpl;
			// 3. 【关键调用】根据资源ID，查询资源的信息，并将结果填充到value容器中。
        	// 第三个参数`true`表示需要解析引用（例如，如果这个资源是@string/hello，我们需要拿到它真正的字符串值）。
            // cpp调用链为 android_util_AssetManager.cpp NativeGetResourceValue() -> CopyValue()
            // 实际作用设置基础参数，包括 type数据类型、assetCookie字段(哪个apk包)、string(全局字符串池中的索引)、resourceId资源ID、changingConfigurations配置变更字段(语言、屏幕方向)、density密度
            // 这一步决定了调用 getString(), getDrawable(), getLayout(), getColor() 等具体资源加载方法的共同前提和基础
            impl.getValue(id, value, true);
			// 4. 检查获取到的资源类型。这里要加载的是XML资源，所以底层资源的值应该是一个字符串，这个字符串就是XML文件的路径（如res/layout/main.xml）。
            if (value.type == TypedValue.TYPE_STRING) { // 这个type实际上是存在xml的二进制文件中,在jni中读取出来赋值
				// 5. 如果确实是字符串，调用另一个重载方法，传入获取到的文件路径、ID等信息，去真正地解析这个XML文件。
                // 细节：value.string 在cpp中实际是null，真实的调用是通过 AssetManager.getResourceValue() 方法里 outValue.string = getPooledStringForCookie(cookie, outValue.data) 去获取的
                return loadXmlResourceParser(value.string.toString(), id,
                        value.assetCookie, type);
            }
			// 6. 如果获取到的资源类型不是字符串（比如是一个颜色值0xFF0000FF），则抛出异常，因为它不能被当作XML解析。
            throw new NotFoundException("Resource ID #0x" + Integer.toHexString(id)
                    + " type #0x" + Integer.toHexString(value.type) + " is not valid");
        } finally {
			// 7. 无论成功与否，释放临时的TypedValue对象回池中，避免频繁创建对象，优化性能。
            releaseTempTypedValue(value);
        }
    }
